...

{
    ...
    two_col_row(ui, |ui| {
        ...
        c1 = ui.add(egui::DragValue::new(&mut px)
            .speed(0.5).suffix(" px").range(-1920.0..=1920.0)).changed();
    }, |ui| {
        ui.label(egui::RichText::new("Y").color(TEXT_DIM).size(10.5));
        circle_dot(ui, dot_color);
        c2 = ui.add(egui::DragValue::new(&mut py)
            .speed(0.5).suffix(" px").range(-1080.0..=1080.0)).changed();
    });
    changed |= c1 | c2;
}
...

{
    ...
    two_col_row(ui, |ui| {
        ...
        circle_dot(ui, dot_color);
        c1 = ui.add(egui::DragValue::new(&mut pscale)
            .speed(0.005).range(0.0..=10.0)).changed();
    }, |ui| {
        ui.label(egui::RichText::new("Rot°").color(TEXT_DIM).size(10.5));
        circle_dot(ui, dot_color);
        c2 = ui.add(egui::DragValue::new(&mut prot)
            .speed(0.5).suffix("°").range(-360.0..=360.0)).changed();
    });
    changed |= c1 | c2;
}
...
prop_row(ui, "Opacity", |ui| {
    ...
    // Use ctx.dragged_id() instead of the deprecated is_anything_being_dragged
    if ctx.dragged_id().is_some() { changed = true; }
});
...
{
    // Snapshot limit values before the two closures so neither closure
    // needs to borrow both pstart and pend simultaneously.
    let pend_limit   = pend;
    let pstart_limit = pstart;
    let (mut sc1, mut sc2) = (false, false);
    two_col_row(ui, |ui| {
        ui.label(egui::RichText::new("Start").color(TEXT_DIM).size(10.5));
        sc1 = ui.add(egui::DragValue::new(&mut pstart)
            .speed(0.05).suffix("s").range(0.0..=(pend_limit - 0.05))).changed();
    }, |ui| {
        ui.label(egui::RichText::new("End").color(TEXT_DIM).size(10.5));
        sc2 = ui.add(egui::DragValue::new(&mut pend)
            .speed(0.05).suffix("s").range((pstart_limit + 0.05)..=3600.0)).changed();
    });
    start_changed |= sc1;
    end_changed   |= sc2;
}
...
// ── Write back ────────────────────────────────────────────────────────────
if changed || start_changed || end_changed {
    if let Some(sub) = vm.selected_subtitle_mut() {
        sub.x        = px;
        sub.y        = py;
        sub.scale    = pscale;
...
if changed && vm.keyframe_mode == KeyframeMode::Record {
